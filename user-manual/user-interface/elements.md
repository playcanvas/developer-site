...
An element is a rectangle in a user interface. Every image, piece of text and group in an interface is an entity with an [element](https://developer.playcanvas.com/user-manual/editor/scenes/components/element.md) component, and it is laid out relative to its parent element, or to the [screen](https://developer.playcanvas.com/user-manual/user-interface/screens.md) if it sits directly below one. This page covers the layout that all elements share. What an element draws is covered on the [Image Elements](https://developer.playcanvas.com/user-manual/user-interface/image-elements.md) and [Text Elements](https://developer.playcanvas.com/user-manual/user-interface/text-elements.md) pages.
...
| Type | Draws | Used for |
| --- | --- | --- |
| **Group** (`pc.ELEMENTTYPE_GROUP`) | Nothing | Containers, [layout groups](https://developer.playcanvas.com/user-manual/user-interface/layout-groups.md) and invisible input areas. See [Group Elements](https://developer.playcanvas.com/user-manual/user-interface/elements.md#group-elements) |
| **Image** (`pc.ELEMENTTYPE_IMAGE`) | A solid color, a texture or a sprite | Panels, icons, buttons and [masks](https://developer.playcanvas.com/user-manual/user-interface/masks.md). See [Image Elements](https://developer.playcanvas.com/user-manual/user-interface/image-elements.md) |
| **Text** (`pc.ELEMENTTYPE_TEXT`) | A string, in a font asset | Labels and any other text. See [Text Elements](https://developer.playcanvas.com/user-manual/user-interface/text-elements.md) |
...
[Image: Nine small rectangles inside a larger panel: one in each corner, one at the middle of each edge and one in the center. A diamond marks the anchor point of each]

The Editor's **Preset** field sets the common combinations for you. Each preset except **Stretch** comes in two versions: *Anchor* sets the anchor only, and *Anchor & Pivot* also moves the [pivot](https://developer.playcanvas.com/user-manual/user-interface/elements.md#pivot) to the same point, so that the element sits inside the corner or edge rather than centered on it:
...
[Image: Three copies of the same rectangle, each rotated by 20 degrees about a different pivot: its bottom-left corner, its center and its top-right corner]
...
Changing an anchor keeps the element's current [margins](https://developer.playcanvas.com/user-manual/user-interface/elements.md#margin), so set the margins of a newly split axis as well, as `left` and `right` do here. The same applies when you create an element with a split anchor: without a `margin`, it starts with the default margins of `0, 0, -32, -32`, the edges of the default 32 × 32 size, and overhangs its right and top anchors by 32 units.

[Image: The same two children in a wide and a narrow parent: a panel that fills the parent apart from a margin on every side, and a bar stretched across its top. Both follow the width of the parent]
...
- **`calculatedWidth` and `calculatedHeight`** are the size the element actually has. They differ from `width` and `height` on an axis where the anchor is split, and when a [layout group](https://developer.playcanvas.com/user-manual/user-interface/layout-groups.md) sizes the element.

A [text element](https://developer.playcanvas.com/user-manual/user-interface/text-elements.md#sizing-wrapping-and-line-limits) can also set its own size to fit its text. Read the calculated size whenever you need the size an element is shown at, for example to fit a background to a label:
...
- **Layout.** A [layout group](https://developer.playcanvas.com/user-manual/user-interface/layout-groups.md) is usually a group element that arranges its children.
- **Invisible input areas.** A group with input enabled receives input across its whole rectangle, which is how the content of a [scroll view](https://developer.playcanvas.com/user-manual/user-interface/scroll-views.md) can be dragged by its empty space.

The color and opacity of an element apply to that element only. They do not cascade to its children, so fading a panel means fading every image and text element below it. See [Common Widgets](https://developer.playcanvas.com/user-manual/user-interface/common-widgets.md#animating-ui).

## Rotation, Scale and Depth
...
The z position has no effect on a screen-space screen, where the hierarchy decides what is drawn on top (see [Draw Order and Performance](https://developer.playcanvas.com/user-manual/user-interface/draw-order-and-performance.md)). On a [world-space screen](https://developer.playcanvas.com/user-manual/user-interface/world-space-ui.md), it moves an element off the plane of the screen.

## Reading an Element's Bounds
...
| `canvasCorners` | CSS pixels, from the top-left corner of the canvas. For elements on screen-space screens | Place [HTML](https://developer.playcanvas.com/user-manual/user-interface/html-and-css.md#over-an-element) over an element, or compare it with mouse positions |
...
## Elements Without a Screen

An element does not need a screen. One with no screen ancestor is placed like any other entity, by its transform, and its width and height are in world units, so a 32 × 32 element is 32 meters across. Its anchors don't move it, but split anchors still size it against a parent element: anchored `0, 0, 1, 1` inside a 100 × 100 element, it is 100 × 100, and anchored `0, 0, 0.5, 0.5`, it is 50 × 50. A lone element suits a single label or image in the world. For anything more, such as a panel with a layout or a menu, use a [world-space screen](https://developer.playcanvas.com/user-manual/user-interface/world-space-ui.md).

## See Also

- [Image Elements](https://developer.playcanvas.com/user-manual/user-interface/image-elements.md) - Colors, textures, sprites and 9-slicing
- [Text Elements](https://developer.playcanvas.com/user-manual/user-interface/text-elements.md) - Text layout, wrapping and effects
- [Layout Groups](https://developer.playcanvas.com/user-manual/user-interface/layout-groups.md) - Arranging children in rows, columns and grids
