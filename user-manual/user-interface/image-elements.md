...
An image element draws a rectangle: a solid color, a texture, or a frame of a sprite. Panels, backgrounds, icons, buttons and [masks](https://developer.playcanvas.com/user-manual/user-interface/masks.md) are all image elements.
...
The color and opacity of an element don't affect its children. See [Group Elements](https://developer.playcanvas.com/user-manual/user-interface/elements.md#group-elements).
...
[Image: The same landscape texture in three square elements, whose rectangles are outlined. Stretch fills the square and distorts the texture, Contain fits the texture inside the square with empty space above and below it, and Cover fills the square and overflows its sides]
...
| **Cover** | Scaled to cover the element, overflowing its sides or its top and bottom. Crop the overflow with a [mask](https://developer.playcanvas.com/user-manual/user-interface/masks.md) |
...
A sprite is a set of frames from a [texture atlas](https://developer.playcanvas.com/user-manual/2D/sprite-editor.md), a texture with named rectangles on it. Assign a sprite asset to an image element to draw its first frame, and set `spriteFrame` to pick another. Sprites also add render modes that keep the borders of a frame crisp at any size.

### 9-Sliced and Tiled Sprites

A sprite whose render mode is **Sliced** keeps the borders of its frame at a fixed size, and stretches the rest to the size of the element. **Tiled** repeats the rest instead of stretching it. One small image then makes panels, buttons and frames of any size. See [9-Slicing](https://developer.playcanvas.com/user-manual/2D/slicing.md) for how the borders of a frame are defined.

[Image: A panel drawn with the Simple, Sliced and Tiled render modes at the same size. Simple stretches the whole image and blurs its rounded corners, Sliced keeps the corners sharp and stretches the middle, and Tiled keeps the corners and repeats the middle]
...
In the [Sprite Editor](https://developer.playcanvas.com/user-manual/2D/sprite-editor.md), set the borders of a frame and click **New Sliced Sprite From Selection**. Then drag the new sprite asset onto the image element's **Sprite** field.
...
Declare the texture atlas before the sprite that uses it. See [`<pc-asset>`](https://developer.playcanvas.com/user-manual/web-components/tags/pc-asset.md).

[Live example: Panel](https://playcanvas.com/examples/#/user-interface/panel) ([source](https://github.com/playcanvas/engine/blob/main/examples/src/examples/user-interface/panel.example.mjs))

## Rendering 3D into an Image
...
[Live example: Custom Shader](https://playcanvas.com/examples/#/user-interface/custom-shader) ([source](https://github.com/playcanvas/engine/blob/main/examples/src/examples/user-interface/custom-shader.example.mjs))
...
- [Masks](https://developer.playcanvas.com/user-manual/user-interface/masks.md) - Clipping children to an element's rectangle or shape
- [Buttons](https://developer.playcanvas.com/user-manual/user-interface/buttons.md) - Changing an image's tint or sprite on hover and press
- [9-Slicing](https://developer.playcanvas.com/user-manual/2D/slicing.md) and [Sprite Editor](https://developer.playcanvas.com/user-manual/2D/sprite-editor.md) - Creating sprites and their borders in the Editor
- [Element Component](https://developer.playcanvas.com/user-manual/editor/scenes/components/element.md), [`<pc-element>`](https://developer.playcanvas.com/user-manual/web-components/tags/pc-element.md) and [ElementComponent](https://api.playcanvas.com/engine/classes/ElementComponent.html) - Reference for every image property
