...
This will allow you to download an app which you can self host on your own server. The request will start a download job and the job details will be returned in the response. You can [poll the job by id](https://developer.playcanvas.com/user-manual/api/job-get.md) until its status is either 'complete' or 'error'. When the job is done, its data will contain a URL to download the app package.

Use `format` to choose the package type. Omit it or set it to `static` for the standard self-hostable package, or set it to `npm` for a Vite-based npm project. See [Downloading Projects](https://developer.playcanvas.com/user-manual/editor/projects/downloading.md#download-formats) for details on the available download formats and [NPM project structure](https://developer.playcanvas.com/user-manual/editor/projects/downloading.md#npm-project-structure).
...
| `optimize_scene_format` | `boolean`  |          | Set it to true if you want scenes to be in an optimized format (see [Optimize Scene Format](https://developer.playcanvas.com/user-manual/optimization/optimizing-scene-format.md) for more information). |
| `format`                | `string`   |          | Download package type: `static` or `npm`. Defaults to `static`. See [Downloading Projects](https://developer.playcanvas.com/user-manual/editor/projects/downloading.md#download-formats).                 |
...
This route uses a [strict](https://developer.playcanvas.com/user-manual/api.md#rate-limiting) rate limit.
