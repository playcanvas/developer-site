...
The file extension you choose determines the type of script. A name without an extension (or ending in `.js`) creates a classic script, while a `.mjs` extension (e.g. `PlayerController.mjs`) creates a modern **ESM** script. ESM is the recommended approach for new projects — see [ESM Scripts](https://developer.playcanvas.com/user-manual/scripting/esm-scripts.md#creating-esm-scripts) for details.
...
* **Importing Scripts from the PlayCanvas Asset Store:**
    1. Open the [Asset Store](https://developer.playcanvas.com/user-manual/editor/assets/asset-store.md) (accessible via a button in the Asset Panel header).
...
Scripts are listed here by their *registered name* (set in code with `static scriptName`, or `pc.createScript('name')` for classic scripts), which may differ from the script asset's file name. See [Script Panel Controls](https://developer.playcanvas.com/user-manual/editor/scenes/components/script.md#script-panel-controls) for details.
...
There is no undo for Asset Deletion so be cautious. Using [version control](https://developer.playcanvas.com/user-manual/editor/version-control.md) to regularly set checkpoints is highly recommended to recover accidentally deleted files.
