...
A Sprite is a 2D graphic that can be rendered into a scene. A Sprite asset is a reference to a [Texture Atlas](https://developer.playcanvas.com/user-manual/editor/assets/inspectors/texture-atlas.md) and a sequence of frames from that atlas. In this way a sprite can either represent a single image (taken out of the atlas) or a flip-book style animation (multiple frames from the atlas).
...
| Render Mode | Controls how the sprite is rendered: **Simple** - the sprite does not use border values; **Sliced** - uses border values to perform [9-sliced](https://developer.playcanvas.com/user-manual/2D/slicing.md) rendering by stretching; **Tiled** - uses border values to perform [9-sliced](https://developer.playcanvas.com/user-manual/2D/slicing.md) rendering by tiling. |
