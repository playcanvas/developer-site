---
title: Draw Order and Performance
description: How interfaces are drawn, on the UI layer and by which cameras, the order of elements, screens and particles, 3D over UI, world-space UI with post-processing, and keeping interfaces fast with batching, cheap text updates, layouts, fill rate and culling.
---
...
## How UI Is Drawn {#how-ui-is-drawn}
...
- **It holds transparent objects only.** Its opaque part is not in the layer order, so a material that doesn't blend is never drawn on it. See [Custom Materials](/user-manual/user-interface/image-elements/#custom-materials).
- **It is sorted by the elements' draw order,** which follows the hierarchy, rather than by distance. See [Draw Order](#draw-order).
...
- **Post-processing doesn't apply to it.** A camera with a [CameraFrame](/user-manual/graphics/posteffects/cameraframe/) draws the UI layer after its effects, so they don't blur, bloom or tone map the interface.
...
![Three overlapping cards, blue, orange and green from first to last in the hierarchy, each drawn over the one before. The orange card's child, a white badge, is drawn over the orange card but under the green one](/img/user-manual/user-interface/draw-order-and-performance/draw-order.webp)
...
In React, a new `<Entity>` is added after its existing siblings, wherever it is in the JSX. See [Creating a Layout Group](/user-manual/user-interface/layout-groups/#creating-a-layout-group) for keeping a list in order.

### Particles in an Interface {#particles}
...
## Multiple Screens and Priority {#multiple-screens}
...
### 3D over the Interface {#3d-over-ui}

A 3D model can be drawn over an interface in two ways. Render it into a texture that an image element shows, which keeps it in the interface's draw order and masks. See [Rendering 3D into an Image](/user-manual/user-interface/image-elements/#render-to-texture). Or put it on a layer after the UI layer that clears the depth buffer, so that the scene doesn't hide it:
...
## World-Space UI and Post-Processing {#world-space-ui}
...
Each image and text element is a draw call. Batching combines elements into fewer draw calls when they share a batch group and a material, and are on the same layer. See [Batching](/user-manual/graphics/advanced-rendering/batching/) for the rules.
...
- **Texture atlases.** Images with different textures can't be batched together. Draw icons and panels as sprites from one [texture atlas](/user-manual/2D/sprite-editor/) instead.
- **Masks.** Elements inside a mask are batched separately from elements outside it. See [Masks](/user-manual/user-interface/masks/#cost).
...
Revealing text with `rangeStart` and `rangeEnd` doesn't lay it out again, so it is cheaper than changing `text`. See [Revealing Text](/user-manual/user-interface/text-elements/#revealing-text).

## Layout Costs {#layout-costs}

A [layout group](/user-manual/user-interface/layout-groups/) lays its children out again whenever one of them is added, removed, enabled, disabled or resized. Animating the size of a child every frame therefore lays the whole group out every frame. Keep animated elements out of layout groups, or give them a layout child with **Exclude from Layout** on.

## Fill Rate {#fill-rate}

Every pixel of an element is drawn and blended, including its transparent pixels. Large overlapping images, such as a full-screen backdrop, a vignette and a panel on top of each other, draw each pixel several times, which costs the most on mobile devices at high pixel ratios. Keep large transparent images few, crop images to their visible content, and see [Pixel Ratio](/user-manual/user-interface/screens/#pixel-ratio) for the resolution you render at.

## Culling {#culling}

A screen-space screen skips drawing the elements that are completely outside the camera's view, as its `cull` property is on. Elements inside a mask are also skipped when they are completely outside the mask, such as the items of a long list that are scrolled out of the viewport of a [scroll view](/user-manual/user-interface/scroll-views/). `cull` is set from `screenSpace` when the screen is created, so passing it to `addComponent` has no effect: set it afterwards. Elements on world-space screens are culled against the camera's view, like the rest of the scene.

## See Also

- [Screens](/user-manual/user-interface/screens/) - Screen space and world space, resolution and priority
- [World-Space UI](/user-manual/user-interface/world-space-ui/) - Interfaces in the scene
- [Batching](/user-manual/graphics/advanced-rendering/batching/) - How batch groups combine draw calls
- [Layers](/user-manual/graphics/layers/) - Layers and the order they are drawn in
