---
title: Buttons
description: Make clickable buttons with the button component, change their tint or sprite as they are hovered and pressed, listen for clicks and state changes, grow their hit area, disable them, and handle touch and XR input.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

A button component turns an element into a button. It reacts to input on its entity's element, changes the look of an image as the button is hovered and pressed, and fires events such as `click`. Its element needs [input](/user-manual/user-interface/input/) enabled.
...
This uses the `Button` component from [Your First Interface](/user-manual/user-interface/user-interface-basics/#your-first-interface), which adds a button component to the entity it is placed in and calls `onClick` when it is clicked. Its tints are `Color` objects from `playcanvas`, as it passes them to the engine unchanged:
...
The input events receive the same event objects as the element's own events. See [Listening for Events](/user-manual/user-interface/input/#listening-for-events) for where to listen on each surface.
...
## Touch and XR {#touch-and-xr}

- **Touch.** A touch has no hover state: a tapped button goes from the default state to pressed and back. On `touchend`, the button cancels the browser's emulated mouse events for the tap, which would otherwise put it in the hovered state. A button that hides itself in its `click` handler misses that `touchend`, so the emulated events reach whatever is behind it. See [Touch Screens](/user-manual/user-interface/input/#touch-screens) for the fix.
- **XR.** A controller or hand pointing at a button hovers it, and a select presses and clicks it. See [UI in XR](/user-manual/user-interface/xr/).

## Sound and Cursor {#sound-and-cursor}

Buttons have no sound or cursor of their own. Play a sound from the `click` event, for example from a [sound component](/user-manual/editor/scenes/components/sound/) on the button, and change the cursor on hover:
...
- [Input](/user-manual/user-interface/input/) - Input events, bubbling and keeping clicks away from the game
- [Common Widgets](/user-manual/user-interface/common-widgets/) - Toggles, radio groups, dialogs and more, built from buttons
- [User Interface - Buttons](/tutorials/ui-elements-buttons/) - Tutorial that builds tinted and sprite buttons in the Editor
- [Button Component](/user-manual/editor/scenes/components/button/), [`<pc-button>`](/user-manual/web-components/tags/pc-button/) and [ButtonComponent](https://api.playcanvas.com/engine/classes/ButtonComponent.html) - Reference for every button property
