...
description: Recipes for progress bars, sliders, toggles, radio groups, modal dialogs, tooltips, dynamic lists, drag and drop, text fields, animated UI, loading screens and touch joysticks, built from elements, buttons, layout groups and scrollbars.
---
...
This squashes a textured fill. To crop the texture instead, anchor the fill to the track's left edge, with its pivot there too, and set its `width` and its `rect` together: `fill.element.width = value * 292`, the width of the track, and `fill.element.rect = new pc.Vec4(0, 0, value, 1)`. See the [progress bar](/tutorials/ui-elements-progress/) and [loading circle](/tutorials/loading-circle-ui/) tutorials for more.

## Sliders {#sliders}

A [scrollbar](/user-manual/user-interface/scroll-views/#scrollbars) whose handle is smaller than its track is a slider. Its `value` goes from 0 to 1 as the handle is dragged, and on a vertical scrollbar, 0 is at the top.
...
## Toggles and Radio Groups {#toggles}
...
To show a dialog over several screens, put it on a screen of its own with a higher [priority](/user-manual/user-interface/draw-order-and-performance/#multiple-screens).
...
A list whose items come from data, such as a leaderboard or an inventory, is a [layout group](/user-manual/user-interface/layout-groups/) whose children are created from the data. Put it in a [scroll view](/user-manual/user-interface/scroll-views/) when it can grow beyond its area.
...
Build one row in the Hierarchy, with two text elements named `name` and `score` below it, and turn it into a [template](/user-manual/editor/templates/). Then create the rows from a script:
...
`LayoutGroup` is the component from [Creating a Layout Group](/user-manual/user-interface/layout-groups/#creating-a-layout-group), which also explains how to keep the rows in order when items are inserted.
...
The UI system has no text input element. For text entry, place an HTML `<input>` over the canvas, which also gives the player the keyboard, selection, autocomplete and accessibility of the browser. See [HTML and CSS](/user-manual/user-interface/html-and-css/#over-an-element), and the [UI Text Input](/tutorials/ui-text-input/) tutorial for a field that looks like the rest of an in-canvas interface.

## Animating UI {#animating-ui}
...
This sets the same opacity on every element, so store the elements' own opacities first if they differ. Buttons fade between their tints with `fadeDuration`. See the [Tweening](/tutorials/tweening/) tutorial for a tween library.

## Loading Screens and Scene Changes {#loading-screens}

- **Loading screens.** An Editor project shows its loading screen before any scene, and so before any UI, has loaded, so the loading screen is HTML and CSS. See [Loading Screen](/user-manual/editor/interface/launch-page/loading-screen/).
- **Scene changes.** An interface that belongs to a scene is destroyed with the scene. To keep a HUD across scene changes, load the new scene's hierarchy next to it rather than replacing everything. See [Loading Scenes](/user-manual/editor/scenes/loading-scenes/) and the [Changing Scenes](/tutorials/changing-scenes/) tutorial.

## Touch Joysticks {#touch-joysticks}

On-screen joysticks and buttons for touch devices are elements that read touch events. The [Touchscreen Joypad Controls](/tutorials/touch-joypad/) tutorial builds a twin-stick layout that you can reuse.

## See Also

- [Buttons](/user-manual/user-interface/buttons/) - States, tints and events
- [Layout Groups](/user-manual/user-interface/layout-groups/) - Arranging lists and grids
- [Scroll Views](/user-manual/user-interface/scroll-views/) - Scrolling content and scrollbars
- [HTML and CSS](/user-manual/user-interface/html-and-css/) - Interfaces built with the DOM
