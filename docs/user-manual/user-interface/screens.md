---
title: Screens
description: Choose between screen-space and world-space screens, scale an interface to fit any canvas with Blend and a reference resolution, and handle pixel ratio, orientation, multiple screens and hiding an interface.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

A screen is the root of an interface. It is an entity with a [screen](/user-manual/editor/scenes/components/screen/) component, and the [elements](/user-manual/user-interface/elements/) below it are laid out in its space. The screen decides whether the interface is drawn over the camera's view or placed in the scene, what its units measure, and how it grows and shrinks with the canvas.

## Screen Space and World Space {#screen-space-and-world-space}

![A 3D scene with a panel standing in it, seen in perspective, and a flat HUD with a bar and buttons drawn over the view](/img/user-manual/user-interface/screens/screen-and-world-space.webp)
...
A **world-space** screen is placed in the scene by its entity's transform and drawn with the rest of the scene: a sign on a wall, a panel above a machine or a menu in [XR](/user-manual/user-interface/xr/). Its size is its `resolution`, in the entity's local units, centered on the entity. See [World-Space UI](/user-manual/user-interface/world-space-ui/).
...
## Units and Resolution {#units-and-resolution}
...
| Screen space, **Scale Mode** Blend | The same | `scale` pixels of the drawing buffer. See [Scaling with Blend](#scaling-with-blend) |
...
## Scaling with Blend {#scaling-with-blend}
...
![The same interface on a small and a large canvas. With Scale Mode None, in the top row, it overflows the small canvas and looks small on the large one. With Blend, in the bottom row, it is scaled to fit both](/img/user-manual/user-interface/screens/scale-modes.webp)
...
### Choosing the Scale Blend {#scale-blend}
...
![A landscape and a portrait canvas for each Scale Blend: 0 on the left, 0.5 in the middle and 1 on the right. In portrait, 0 makes the interface small enough to fit the width, 0.5 keeps it the size it has in landscape, and 1 makes it too wide to fit](/img/user-manual/user-interface/screens/scale-blend.webp)
...
### Pixel-Perfect Images {#pixel-perfect-images}
...
- Anchor each element to the corner or edge it belongs to, so that it follows that edge on a wider or taller canvas, and use split anchors for bars and panels that should stretch. See [Elements](/user-manual/user-interface/elements/#anchor).
- Keep centered content within the smallest canvas you support, or keep the whole reference area on screen as in [Choosing the Scale Blend](#scale-blend).
- On phones, keep essential elements inside the [safe area](/user-manual/user-interface/safe-area/).
- Let text wrap or shrink to fit its element. See [Text Elements](/user-manual/user-interface/text-elements/#sizing-wrapping-and-line-limits).
- Test at the smallest and largest canvases you support, in both orientations, and at pixel ratios of 1 and 2 or more.

## Multiple Screens {#multiple-screens}

An application can have several screens, for example a HUD, a pause menu and a sign in the world, and each lays out its own elements. When screens overlap, the one with the higher `priority`, from 0 to 127, is drawn on top and tested first for input. See [Draw Order and Performance](/user-manual/user-interface/draw-order-and-performance/#multiple-screens).
...
- [World-Space UI](/user-manual/user-interface/world-space-ui/) - Screens in the scene, labels over objects and facing the camera
- [Safe Areas](/user-manual/user-interface/safe-area/) - Keeping an interface clear of notches and rounded corners
- [Draw Order and Performance](/user-manual/user-interface/draw-order-and-performance/) - Priorities, layers and cameras
