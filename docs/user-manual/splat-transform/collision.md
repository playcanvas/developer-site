...
description: "Generate sparse voxel octrees and collision meshes from Gaussian splats with splat-transform."
---

This guide explains how to generate collision data from a Gaussian splat scene using [splat-transform](/user-manual/splat-transform/). It covers both **voxel generation** (a sparse voxel octree, `.voxel.json` + `.voxel.bin`) and **mesh generation** (`.collision.glb`) suitable for runtime collision detection.

For uploading or generating voxel collision through the SuperSplat web UI instead of the CLI, see [Studio → Collision](/user-manual/supersplat/studio/collision).
...
<!-- TODO: media — diagram of cross-section before/after external-fill (upstream guides/images/external-fill.png is missing). -->
