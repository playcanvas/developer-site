...
The PlayCanvas Engine requires a browser with [WebGL 2.0](https://en.wikipedia.org/wiki/WebGL#WebGL_2) support and the JavaScript features described in [Build Target](#build-target). The minimum browser versions are:
...
We recommend using an up-to-date browser for the best performance and access to newer features like [WebGPU](#webgpu).
