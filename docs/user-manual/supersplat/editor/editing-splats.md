...
| ![Polygon Select](/img/user-manual/supersplat/editor/select-poly.svg) **Polygon Select** | Click to place points that define the vertices of a polygon. Double-click to close the shape. Splats within the polygon will be selected. This is useful for precise selections with straight edges. |
