...
By default, the PlayCanvas Engine does not support "global sorting" across multiple GSplatComponents (where all Gaussians from all components would be sorted together). However, you can enable **global sorting** by using [Unified Rendering](/user-manual/gaussian-splatting/building/engine-features/unified-rendering), a beta feature that allows all Gaussians from multiple components to be sorted together, eliminating visibility and popping artifacts.
