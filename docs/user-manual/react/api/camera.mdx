...
Without a camera in your scene, nothing will be rendered. The `<Camera/>` component enables an [`<Entity/>`](../entity) to render a scene from its perspective. Set the entity's position and rotation to control the camera's viewpoint.
