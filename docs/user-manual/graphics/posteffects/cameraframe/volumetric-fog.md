...
description: Set up CameraFrame height fog and light shafts, tune directional and local lights, and handle performance and Gaussian splat depth.
---
...
Local lights require [clustered lighting](/user-manual/graphics/lighting/clustered-lighting/), which is enabled by default. Enable the light types you want to contribute:
...
Add and parse [`camera-frame.mjs`](https://github.com/playcanvas/engine/blob/main/scripts/esm/camera-frame.mjs), then attach the registered `cameraFrame` script to the camera entity as described in [CameraFrame in the Editor](/user-manual/graphics/linear-workflow/hdr-rendering/#cameraframe-in-the-editor).
