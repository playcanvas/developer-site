...
The `<Render/>` component allows an [`<Entity/>`](./entity) to display a 3D asset or primitive shape. Adding the `<Render/>` component to an [`<Entity/>`](./entity) will add a `RenderComponent` to the entity. The `type` prop allows you to select the primitive shape to render.
