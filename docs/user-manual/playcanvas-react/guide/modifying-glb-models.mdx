...
function ConditionalPhysics() {
  // Only add physics if a specific mesh exists
  const bodyMesh = useEntity('Body.Mesh');
...
**Finding entities to add effects:**
```tsx
function AddEffectsToLights() {
  // Find all lights in the current subtree
  const lights = useEntity('**[light]');
  ...
  return (
    <>
      {lightArray.map((light, index) => (
        <Entity key={index} name={`LightEffect_${index}`}>
          {/* Add effects relative to each light */}
        </Entity>
      ))}
    </>
  );
...
**Finding specific child entities:**
```tsx
function AddHandAccessory() {
  // Find the Hand entity within the Arm subtree
  const hand = useEntity('Hand');
  
  if (!hand) return null;
  
  // Add an accessory to the hand
  return (
    <Entity name="Ring" position={[0, 0.05, 0]}>
      <Render type="torus" />
    </Entity>
  );
}
