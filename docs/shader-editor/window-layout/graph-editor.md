---
title: Graph Editor
description: The Graph Editor workspace for creating, connecting, moving, and deleting nodes while editing the selected shader graph.
sidebar_position: 5
---

The Graph Editor displays the currently selected graph. This is where nodes are created, arranged, connected and deleted.

![Graph Editor](pathname:///img/shader-editor/graph-editor.png)

| How To | |
|---|---|
| Create a node | Drag an entry from the [Nodes Pane](/shader-editor/window-layout/nodes-pane) and drop it on the Graph Editor work area.<br />Drag a sub-graph from the [Assets Pane](/shader-editor/window-layout/assets-pane) onto the Graph Editor work area. |
| Select a node | Left click the node. |
| Delete a node | Select the node and either press delete key or use the node context menu 'Delete' option. |
| Move a node | Left click and drag the node. |
| Modify a node | Select the node and modify its properties in the [Inspector Pane](/shader-editor/window-layout/inspector-pane). |
| Connect two nodes | Left click the output port of the source node, drag and drop on the input port of the target node. |
