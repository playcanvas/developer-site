...
* **[Engine Development](/user-manual/ai/developing-with-engine/):** 「Device Pixel Ratio」について、次の要件を満たしてください: ランタイムで maxPixelRatio を変更し、さまざまな端末でシャープなレンダリングとフィルレート制限のバランスを取ります 変更前後のプロファイラー、ロード、メモリ、またはフレーム時間の該当指標を測定してください。
* **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** `maxPixelRatio` を制御するプロジェクト設定または Script を更新し、MiniStats を有効にして Launch して、変更前後のフレーム時間と見た目の鮮明さを比較します。
