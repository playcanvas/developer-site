---
title: GPUプロファイリング
description: デスクトップと Android のネイティブ GPU プロファイラで WebGL または WebGPU のフレームを取得し、GPU を深く分析します。
---

:::ai

* **[Engine Development](/user-manual/ai/developing-with-engine/):** 「GPUプロファイリング」について、次の要件を満たしてください: デスクトップと Android のネイティブ GPU プロファイラで WebGL または WebGPU のフレームを取得し、GPU を深く分析すること 変更前後のプロファイラー、ロード、メモリ、またはフレーム時間の該当指標を測定してください。
...
* MiniBrowserを起動し、WebアプリケーションのURLを指定します:
    ...
    ```bash
    __XPC_METAL_CAPTURE_ENABLED=1 Tools/Scripts/run-minibrowser --release --url https://playcanvas.com/examples/
    ```
