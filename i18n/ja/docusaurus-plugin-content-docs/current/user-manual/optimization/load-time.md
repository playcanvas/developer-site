---
title: ロード時間の最適化
description: 画像形式、テクスチャサイズ、プリロードの選択、Asset の監査、gzip、段階的ロードで起動を高速化します。
---

:::ai

* **[Engine Development](/user-manual/ai/developing-with-engine/):** 「ロード時間の最適化」について、次の要件を満たしてください: 画像形式、テクスチャサイズ、プリロードの選択、Asset の監査、gzip、段階的ロードで起動を高速化すること 変更前後のプロファイラー、ロード、メモリ、またはフレーム時間の該当指標を測定してください。
* **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** プリロードする Asset、Bundle、Texture サイズ、Script、設定を監査し、最も効果の高い問題を変更してから、プロジェクトを Build または Launch してロード時の証拠を比較します。
