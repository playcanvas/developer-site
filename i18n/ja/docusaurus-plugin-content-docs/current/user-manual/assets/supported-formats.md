...
description: 3D モデル、テクスチャ、オーディオ、フォント、その他の Asset タイプについて、PlayCanvas がサポートするファイル形式の一覧です。
---

:::ai

- **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** 依頼されたソースファイルをアップロードし、生成されたアセットタイプと処理済みファイルを確認して、その形式が正常にインポートされたか報告してください。
...
| SPZ | `.spz` | 圧縮された3D Gaussian Splatデータ ([Niantic SPZ形式](/user-manual/gaussian-splatting/formats/spz))。パーサースクリプトで読み込み |

## カスタムフォーマット

上記のリストはエンジンの組み込みローダーを対象としていますが、固定ではありません。ランタイムでカスタムパーサーを使用すると、ほとんどのアセットタイプで新しいファイルフォーマットのサポートを追加したり、組み込みのものを上書きしたりできます。[カスタムアセットパーサー](custom-parsers.md)を参照してください。

## 関連項目

- [モデル](models/index.md) - PlayCanvas 用の 3D モデルの準備
- [アセットインスペクター](/user-manual/editor/assets/inspectors/) - エディタでアセットプロパティを設定
- [カスタムアセットパーサー](custom-parsers.md) - ランタイムでカスタムファイルフォーマットを読み込む
