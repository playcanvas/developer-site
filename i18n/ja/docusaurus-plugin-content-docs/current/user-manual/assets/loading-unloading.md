...
これはプロセス全体のグローバルな設定です（共有 HTTP レイヤーに適用されます）。そのため、1 つのページに複数のアプリケーションがある場合は、最後に設定された値が優先されます。この設定はすべてのアセットの読み込みに適用されます。これには、HTTP レイヤーを経由せずに直接データを取得する Gaussian Splat やアセットバンドルのローダーも含まれます。
