...
- **[Engine Development](/user-manual/ai/developing-with-engine/):** 「画像トラッキング」について、次の要件を満たしてください: PlayCanvasのARで実世界の画像をトラッキング: 参照画像、推定サイズ、姿勢の更新、実行時のサポート確認であること 対応する XR セッションで実行し、記載された操作を試して、ランタイムログと観察結果を報告してください。
