...
- **[Engine Development](/user-manual/ai/developing-with-engine/):** 「Compose シェーダーのカスタマイズ」について、次の要件を満たしてください: CameraFrame の compose チャンクを上書きして、最終フルスクリーン結合パスにユニフォームとピクセル効果を注入すること アプリケーションを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
- **[VS Code Extension](/user-manual/ai/vscode-extension/):** 関連するスクリプトまたはシェーダーアセットに「Compose シェーダーのカスタマイズ」を実装し、次の要件を満たしてください: CameraFrame の compose チャンクを上書きして、最終フルスクリーン結合パスにユニフォームとピクセル効果を注入すること。Push の前に完全な差分と診断を確認してください。
- **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** 開いているプロジェクトで「Compose シェーダーのカスタマイズ」を設定し、次の要件を満たしてください: CameraFrame の compose チャンクを上書きして、最終フルスクリーン結合パスにユニフォームとピクセル効果を注入すること。シーンを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
