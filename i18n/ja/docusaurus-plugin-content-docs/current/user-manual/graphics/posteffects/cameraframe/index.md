...
- [HDR と Bloom、LUT](https://playcanvas.vercel.app/#/graphics/hdr) — HDR ブルームとカラールックアップテーブルをデモ
- [ポストプロセス](https://playcanvas.vercel.app/#/graphics/post-processing) — ブルーム、グレーディング、ビネット、フリンジ、TAA など複数のエフェクトを表示
- [アンビエントオクルージョン](https://playcanvas.vercel.app/#/graphics/ambient-occlusion) — SSAO の実装をデモ
- [被写界深度](https://playcanvas.vercel.app/#/graphics/depth-of-field) — 被写界深度エフェクトをデモ
- [時間的反エイリアシング](https://playcanvas.vercel.app/#/graphics/taa) — TAA の実装をデモ
...
### [RenderPassCameraFrame クラスの拡張](extending-class)

カスタムレンダーパスを追加して `CameraFrame` を拡張します。組み込みエフェクトを活かしつつ、追加のレンダリング手法を組み込めます。
