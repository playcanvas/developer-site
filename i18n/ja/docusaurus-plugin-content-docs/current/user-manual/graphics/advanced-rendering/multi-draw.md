...
`gl_DrawID`は`WEBGL_multi_draw`拡張機能自体によって提供されるため、フォールバックパスでは利用できません。エンジンは拡張機能が利用可能な場合にのみ`CAPS_MULTI_DRAW`を定義します。そのため、`gl_DrawID`を読み取るシェーダーコードはこれでガードする必要があります。

レンダリングするジオメトリの範囲のみが異なるサブドローは、フォールバックが各範囲を個別のドローとして送信するため、どちらの場合でも同じように動作します。一方、シェーダー内で自身を識別する必要があるサブドローは動作しません。マテリアルが`gl_DrawID`を読み取る場合は、`supportsMultiDraw`を確認し、falseの場合は別のパスを用意してください。以下の「クロスプラットフォームアプローチ」を参照してください。
