...
- **[Engine Development](/user-manual/ai/developing-with-engine/):** 「間接描画」について、次の要件を満たしてください: WebGPUの間接ドロースロット、GPUバッファパラメータ、コンピュート駆動のカリング、フレームごとのスロット割り当てルールであること アプリケーションを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
- **[VS Code Extension](/user-manual/ai/vscode-extension/):** 関連するスクリプトまたはシェーダーアセットに「間接描画」を実装し、次の要件を満たしてください: WebGPUの間接ドロースロット、GPUバッファパラメータ、コンピュート駆動のカリング、フレームごとのスロット割り当てルールであること。Push の前に完全な差分と診断を確認してください。
- **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** 開いているプロジェクトで「間接描画」を設定し、次の要件を満たしてください: WebGPUの間接ドロースロット、GPUバッファパラメータ、コンピュート駆動のカリング、フレームごとのスロット割り当てルールであること。シーンを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
