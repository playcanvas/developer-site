...
## ライトの最大数 {#maximum-number-of-lights}

内部的には、ライトのインデックスは8ビットで保存されているため、デフォルトでは任意のフレームで可視的なライトの最大数は255です。この制限を超えるライトは無視され、コンソールに警告が報告されます。

Engine v2.22以降では、[LightingParamsの maxLights](https://api.playcanvas.com/engine/classes/LightingParams.html#maxlights) を使用してこの制限を引き上げることができます。255を超える値では、ライトのインデックスは代わりに16ビットで保存されるため、3Dグリッドを保存するために使用されるテクスチャのサイズが2倍になります。この制限は、さらにデバイスがサポートするテクスチャの最大サイズによって制限されます。
...
これはエディタの設定では利用できないため、コードで設定する必要があります。
...
- **ライトの最大数** はシーンが許す限り小さく保つべきです。大きな値は毎フレームのコストを伴います。
