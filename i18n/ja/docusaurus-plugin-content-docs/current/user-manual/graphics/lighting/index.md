...
- **[Engine Development](/user-manual/ai/developing-with-engine/):** 「ライティング」について、次の要件を満たしてください: 動的ランタイムライティングとベイク済みライトマップの比較と、PlayCanvas の Scene でそれぞれを使うタイミングであること アプリケーションを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
- **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** 開いているプロジェクトで「ライティング」を設定し、次の要件を満たしてください: 動的ランタイムライティングとベイク済みライトマップの比較と、PlayCanvas の Scene でそれぞれを使うタイミングであること。シーンを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
