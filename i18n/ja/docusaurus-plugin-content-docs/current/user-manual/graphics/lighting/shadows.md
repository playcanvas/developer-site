...
PCSS は、デバイスが浮動小数点テクスチャへのレンダリングと線形フィルタリングをサポートしている必要があります。これは最新のデスクトップ GPU では広くサポートされていますが、特に古い、または低スペックのモバイルデバイスでは、必ずしも利用できるとは限りません。サポートされていない場合、ライトは自動的に PCF にフォールバックするため、PCSS は常に安全に有効化できます。実行時にサポートを検出するには、`GraphicsDevice` の `textureFloatRenderable` および `textureFloatFilterable` プロパティを確認してください。
