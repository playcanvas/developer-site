...
PlayCanvas は、シャドウマッピングと呼ばれる技術を使用してリアルタイムの動的シャドウをレンダリングします。これはすべてのライトタイプでサポートされ、デスクトップからモバイルまですべてのプラットフォームで動作します。このページでは、シャドウを有効にする方法、どのオブジェクトがシャドウをキャスト／受信するかの選択、そしてシャドウカスケードやソフトシャドウを含む品質の調整方法について説明します。
...
シャドウサンプリングタイプは各ライトごとに指定されるため、「Light Inspector（ライトインスペクター）」、またはコードで `light.shadowType` を介して設定できます。PlayCanvas には3つのフィルタリング技術があります。

### PCF (Percentage-Closer Filtering) {#pcf}

PCF はデフォルトの技術です。シャドウマップから複数の局所的なサンプル（例えば 3×3 の9個のサンプル）を読み込んで平均し、エッジを一定量だけ柔らかくします。カーネルサイズは `pc.SHADOW_PCF1_32F`（単一サンプルで最も硬いエッジ）、`pc.SHADOW_PCF3_32F`（デフォルト）、`pc.SHADOW_PCF5_32F` で選択します。サンプル数が多いほどエッジは柔らかくなりますが、GPU コストが高くなります。

### VSM (Variance Shadow Maps) {#vsm}

バリアンスシャドウマップは、事前にブラーをかけられる統計的な深度情報を格納し、Directional ライトのシャドウのような広い範囲に適した滑らかでソフトなエッジを生成します。タイプを `pc.SHADOW_VSM_16F`、またはより高精度な `pc.SHADOW_VSM_32F` に設定します。VSM は一部のシーンでライトブリーディングのアーティファクトが発生することがあります。以下で調整します。

* `light.vsmBlurSize` — ブラーのカーネルサイズ。1 から 25 の奇数。既定値は `11`。
* `light.vsmBlurMode` — `pc.BLUR_GAUSSIAN`（デフォルト、より滑らか）または `pc.BLUR_BOX`（より低コスト）。
...
Hierarchy でライトを選択し、[Lightコンポーネント](/user-manual/editor/scenes/components/light)で **Shadow Type** を **PCSS** に設定します。
...
PCSS はデバイス上での浮動小数点テクスチャのサポートを必要とします。サポートされていない場合、ライトは自動的に PCF にフォールバックします。

PCSS の見た目とコストは、以下のプロパティで微調整できます。コードではライトコンポーネント（`light.penumbraSize`）に、[React](/user-manual/react/) の props として、または Web Components の属性（`penumbra-size`）として設定します。
