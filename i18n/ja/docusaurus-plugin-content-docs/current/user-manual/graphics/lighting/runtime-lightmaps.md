...
## デバイスロスト後の復旧 {#recovery-after-device-loss}

ランタイムでベイクしたライトマップはGPUで生成されるため、デバイスロストで内容が失われます。Engine 2.23以降では、グラフィックスデバイスの `devicerestored` イベントの後に再ベイクを予約してください。詳細は[デバイスロストと復旧](../advanced-rendering/device-loss.md)を参照してください。
