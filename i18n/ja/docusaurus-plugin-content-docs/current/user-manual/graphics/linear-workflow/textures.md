...
- **[Engine Development](/user-manual/ai/developing-with-engine/):** 「テクスチャ」について、次の要件を満たしてください: Asset とコードでカラーテクスチャを sRGB としてマークし（プロシージャルなターゲットも含む）、サンプリングを物理的に正しく保ちます アプリケーションを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
- **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** 開いているプロジェクトで「テクスチャ」を設定し、次の要件を満たしてください: Asset とコードでカラーテクスチャを sRGB としてマークし（プロシージャルなターゲットも含む）、サンプリングを物理的に正しく保ちます。シーンを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
