...
Engine 2.23以降では、デバイスの復旧時にコンピュートリソースが再作成されますが、ストレージバッファの内容は再度書き込むか再生成する必要があります。中断された読み取りへの対応や明示的なリソースの破棄については、[デバイスロストと復旧](../advanced-rendering/device-loss.md)を参照してください。
