...
- **[Engine Development](/user-manual/ai/developing-with-engine/):** 「WGSL 頂点・フラグメントシェーダー」について、次の要件を満たしてください: PlayCanvas における頂点・フラグメント固有の WGSL 構文：属性、バリイング、フラグメント出力 アプリケーションを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
- **[VS Code Extension](/user-manual/ai/vscode-extension/):** 関連するスクリプトまたはシェーダーアセットに「WGSL 頂点・フラグメントシェーダー」を実装し、次の要件を満たしてください: PlayCanvas における頂点・フラグメント固有の WGSL 構文：属性、バリイング、フラグメント出力。Push の前に完全な差分と診断を確認してください。
