...
- **[Engine Development](/user-manual/ai/developing-with-engine/):** 「シーンピッカー」について、次の要件を満たしてください: Picker API で画面座標からメッシュとスプラットを選択し、非同期読み取りと、任意で深度に基づくワールド座標を取得すること アプリケーションを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
- **[VS Code Extension](/user-manual/ai/vscode-extension/):** 「シーンピッカー」で使用する Camera Script を Pull/Push モードでローカル編集し、変更を確認してください。
- **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** 開いているプロジェクトで「シーンピッカー」を設定し、次の要件を満たしてください: Picker API で画面座標からメッシュとスプラットを選択し、非同期読み取りと、任意で深度に基づくワールド座標を取得すること。シーンを起動して表示結果をキャプチャし、シェーダーまたはレンダリングのコンソールエラーを確認してください。
