...
- `GSPLAT_LODMODE_DISTANCE`：誤差メタデータを無視し、カメラを中心とした明確な同心円状のバンドで詳細が段階的に低下することを保証します。バンドの境界は予算に応じて調整されます。キャプチャの誤差データが視覚的な重要度と一致しない場合に便利です。
...
値の範囲は0から8で、デフォルトは1（バランスの取れたフォールオフ）です。値が大きいほど遠方を犠牲にしてカメラ付近に詳細が集中し、0に近づけると視点に関係なくシーン全体に均等に分配されます。これは、そのスプラットが予算から受け取る詳細の配分を変えるもので、予算全体に占めるシェア自体は変わりません。
