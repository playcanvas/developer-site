...
| `meta.json` | ✅ | ✅ | 非バンドル版超圧縮フォーマット (`.webp` テクスチャを伴う)。出力ファイル名は**必ず** `meta.json` で終わる必要があります |
| `lod-meta.json` | ❌ | ✅ | プログレッシブローディング用のオクツリー構造を持つLODストリーミングフォーマット。出力ファイル名は**必ず** `lod-meta.json` で終わる必要があります |
...
出力パスは正確に `lod-meta.json` または `meta.json` で終わる必要があります。例：`output/lod-meta.json`、`my-scene/lod-meta.json`。
