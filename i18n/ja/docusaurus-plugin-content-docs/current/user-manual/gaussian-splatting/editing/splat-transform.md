...
| `lod-meta.json` | ❌ | ✅ | プログレッシブローディング用のオクツリー構造を持つLODストリーミングフォーマット。出力ファイル名は**必ず** `lod-meta.json` である必要があります |
...
# 注: 出力ファイル名はストリーミングLODの場合は lod-meta.json、
# 単一の非バンドル版SOGファイルの場合は meta.json である必要があります
