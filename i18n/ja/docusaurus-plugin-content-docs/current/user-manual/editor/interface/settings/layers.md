...
- **[VS Code Extension](/user-manual/ai/vscode-extension/):** `.pc/settings.json` の「レイヤー設定」に関係する値だけを編集し、次の要件を満たしてください: どのオブジェクトをまとめて、どの順で描画するかを制御するレンダリングレイヤーを管理すること。Push の前に完全な差分と診断を確認してください。
- **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** 現在の「レイヤー設定」の値を読み取り、次の要件に必要な値だけを変更してください: どのオブジェクトをまとめて、どの順で描画するかを制御するレンダリングレイヤーを管理すること。値を再取得し、ランタイムに影響する場合はプロジェクトを起動してください。
