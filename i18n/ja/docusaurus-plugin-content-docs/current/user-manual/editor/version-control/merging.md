---
title: マージとコンフリクトの解決
description: チェックポイント間のマージを開始し、PlayCanvasのマージツールでコンフリクトを解決し、チェックポイント優先のオプションを安全に扱います。
---

マージは、2つのブランチで行われた作業を組み合わせるプロセスです。マージは、[ブランチのドキュメント](/user-manual/editor/version-control/branches)で説明されているブランチベースのワークフローの自然な部分です。
...
![Merging checkpoints](pathname:///img/user-manual/editor/version-control/merging/merging-checkpoints-1.png)
*Changes Y & Z are not included in merge result C and **are lost**.*

![Merging checkpoints](pathname:///img/user-manual/editor/version-control/merging/merging-checkpoints-2.png)
*By default PlayCanvas creates a new checkpoint in the destination branch of a merge, so changes Y are included*

![Merging checkpoints](pathname:///img/user-manual/editor/version-control/merging/merging-checkpoints-3.png)
*If changes Z are required, create a checkpoint in the source branch before starting the merge.*

## マージの開始

![Start merge](pathname:///img/user-manual/editor/version-control/merging/start-merge.png)

マージを開始するには、現在のブランチをマージしたいブランチに切り替えます。次に、マージしたいブランチを選択して、ブランチのドロップダウンメニューから「現在のブランチにマージ」を選択します。

![Merge dialog](pathname:///img/user-manual/editor/version-control/merging/merge-dialog.png)
...
![Conflict Manager](pathname:///img/user-manual/editor/version-control/merging/conflict-manager.jpg)
...
![Resolved Conflicts](pathname:///img/user-manual/editor/version-control/merging/conflicts-resolved.jpg)
