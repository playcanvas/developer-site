...
* **[VS Code Extension](/user-manual/ai/vscode-extension/):** プロジェクトファイルを編集する外部のコーディングアシスタントには Pull/Push を使用してください。Realtime モードは、閉じたファイルへの外部変更を PlayCanvas に送信しません。 Push の前に完全な差分と診断を確認してください。
