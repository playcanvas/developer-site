...
description: ソースファイルをランタイム形式に変換するサーバー側のアセットインポート処理を設定します。関連 Asset の検索、プリロードの既定値、テクスチャのインポートオプションを含みます。
---

<!-- TODO: enable this AI block after editor-mcp-server feat/editor-driver-coverage is released.
:::ai

- **[Editor MCP Server](/user-manual/ai/editor-mcp-server/):** アセットを再インポートし、UV1 座標をアンラップし、テクスチャを変換し、アトラス、キューブマップ、スプライト、バンドルを生成できます。
...
## アセットインポート (Asset Import) {#asset-import}

PlayCanvasは、ソース (Source) アセットがアップロードされると、アセットインポートタスクを開始し、サーバー上でインポートプロセスを実行します。
