---
title: バックアップとエクスポート
description: フォーク、エクスポート、スケジュールダウンロード、REST バックアップ、ストレージ上限、復元を比較し、エクスポートからプロジェクトに何が含まれないかも示します。
---
...
### 2. エクスポート（playcanvas.com経由）

エクスポートファイルは、オフラインで保存し、後でインポートできる完全なプロジェクトバックアップを提供します。
...
[プロジェクトエクスポートAPI](/user-manual/api/project-export/)を使用すると、以下が可能になります：
...
PlayCanvasは、自動バックアップを簡素化するための[オープンソースのNode.jsツール](https://github.com/playcanvas/playcanvas-rest-api-tools#exporting-a-project)を提供しています：
...
- 重要なマイルストーンのためにエクスポートを作成します

📅 **毎週バックアップ**（進行中のプロジェクトの場合）：

- 包括的なプロジェクトエクスポートを作成します
- 定期的に復元手順をテストします

📆 **毎月バックアップ**（安定したプロジェクトの場合）：

- ドキュメント付きの完全なプロジェクトエクスポート
- 長期ストレージ計画
...
- **リリースバックアップ**：すべての安定版リリースをエクスポートする
