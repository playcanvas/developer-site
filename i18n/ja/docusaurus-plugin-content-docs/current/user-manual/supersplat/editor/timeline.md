...
カメラアニメーショントラックは、ここエディターのタイムラインで作成します — [SuperSplat Studio](/user-manual/supersplat/studio/)では作成できません。Studioはカメラ、注釈、ポストエフェクト、スカイボックス、コリジョンのみを調整します。トラックの保存されるJSON形式（`animTracks`、`loopMode`、`interpolation`、`startMode`）については、[Experience Settings](/user-manual/supersplat/studio/experience-settings)を参照してください。
