...
選択に満足したら、**Delete**または**Backspace**で削除できます。
...
- `Delete`または`Backspace`を押して、配置したマーカーを削除します。
- `Escape`キーを押して、測定ツールを無効にします。

## Splatsのマージ

複数のスプラットファイルを1つの`.ply`にマージするには、それらをScene Managerへ読み込み、必要な変換や編集を適用します。含めるすべてのスプラットが表示状態になっていることを確認し、**File > Export > PLY**を選択します。表示されているスプラットが1つのファイルに結合されます。

**File > Save**は、編集可能な`.ssproj`プロジェクトを保存するための操作であり、結合されたPLYはエクスポートしません。詳細は[プロジェクトの管理](managing-projects.md)を参照してください。
